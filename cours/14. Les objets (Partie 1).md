...
Pour notre exemple, une personne aura un nom, un prénom, un age, et à chaque fois qu'elle fête son anniversaire, son age augmente d'un an. On peut ainsi savoir si elle est mineure ou majeure.
...
On va l'appeler `Person` (la majuscule est importante) : %%TODO majuscule importante (convention)%%
![[Pasted image 20251111170344.png]]
...
Ces attributs permettent de décrire comment on représente une personne. C'est une représentation simpliste, vous en conviendrez, mais il vaut mieux commencer simplement non ?

Comment on fait pour manipuler notre nouvelle classe `Person` ? Et bien comme pour n'importe quel autre type, on va déclarer une variable et l'initialiser dans notre `main` !
...
On va ajouter une méthode dans notre classe `Person` :
```java title:Person.java
public class Person {  
...
Un **constructeur** c'est une sorte de méthode qui permet d'initialiser les valeurs d'une **instance** de classe. Pour rappel, une instance, c'est un *exemplaire concret* de la classe. Dans votre main pour l'instant, votre classe `Person` n'a qu'une seule instance stockée dans la variable `personInstance`. 
...
On va donc créer le constructeur de la classe `Person` :
...
En fait, le type de retour est forcément la classe `Person` elle même, donc on ne le précise pas. Aussi, pour appeler un constructeur, il faut utiliser le mot clé `{java}new` (comme vu dans le `main`).
...
Là par contre, vous avez sûrement une erreur qui est apparue dans votre IDE. En effet, dans le `main` on nous signale que la ligne `{java}Person personInstance = new Person();` ne compile plus car il s'attend à trouver 3 arguments dans les parenthèses de `{java}new Person()`. En effet, le constructeur qu'on a écrit nécessite 3 arguments : `{java}String firstName, String lastName, int age`. Alors allons-y, on va lui en donner !
...
>   IO.println("Hello World!");  
  >
  Person personInstance = new Person("Peter", "Parker", 28);  
  IO.println(personInstance);
>}
> ```
...
Je teste dans mon `{java}main` : `{java}IO.println(personInstance.isMinor());`
...
void main() {  
  ...
  Person otherInstance = new Person("Miles", "Morales", 17);  
