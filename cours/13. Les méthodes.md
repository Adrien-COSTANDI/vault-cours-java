...
Et là, les *super-malins* pourraient remarquer que l'[[10. Expression vs instruction (statement)#Expressions|expression]] `{java}n % 2 == 0` renvoie déjà un `{java}boolean`, et que le `{java}if` pourrait être simplifié :
```java
boolean isOddNumber(int n) {
...
Ça ne vous a probablement pas échappé, un nouveau mot clé à fait son apparition : `{java}return`. C'est un mot clé utilisé dans les méthodes pour définir la valeur que la méthode va retourner (son résultat).
Une méthode ne renverra une valeur que si elle a un type de retour différent de `{java}void` (qui signifie l'absence de type). En l'occurence, dans les deux exemples précédents, ma méthode `{java}long square(int value)` a un type de retour `{java}long` donc elle ***retourne*** une valeur de type `{java}long`. Ma méthode `{java}boolean isOddNumber(int n)` ***retourne*** un booléen avec `{java}return n % 2 == 0` qui renvoie soit `true`, soit `false`.

Tout code écrit après un `{java}return` ne sera pas exécuté puisque ce mot clé permet d'arrêter l'exécution de la méthode pour renvoyer immédiatement son résultat.
...
Dans ma méthode `isOddNumber`, j'ai deux fins alternatives : soit le nombre est pair, soit il ne l'est pas. Suivant quelle branche du `{java}if` est exécutée, je renverrai `true` ou `false` mais je ne verrai jamais `Coucou !` s'afficher dans la console !
...
Dans une méthode avec un type de retour `{java}void`, ce mot clé peut être utilisé sans expliciter de valeur, dans le but de quitter la méthode. Il est implicitement présent à la fin de chaque méthode mais vous pouvez le rajouter vous même si vous le souhaitez !
...
Le cas de `main` est un peu spécial puisque comme évoqué dans la partie [[5. Hello World!#Explications|explications sur le Hello World]], il est le **point d'entrée** du programme. Sans lui, pas d'exécution du programme !

Et pour boucler la boucle, on peut même relier toutes ces informations avec le concept précédemment abordé des [[10. Expression vs instruction (statement)|expressions et instructions]]. Car si une méthode renvoie `{java}void`, c'est une **instruction**, sinon, c'est une **expression** ! OMG TOUT EST LIÉ ! (oui)
...
Pour l'instant, vous ne faites qu'utiliser `{java}IO.println()`, qui est une méthode fournie par le [[3. JRE ou JDK#JDK|JDK]] mais c'est loin d'être la seule méthode disponible, et certaines seront utiles pour plus tard alors notons les ici pour pouvoir y revenir si nécessaire :
...
Dans ces cas là, il est essentiel de savoir manipuler des objets. On rentre dans le vrai morceau ! [[14. Les objets (Partie 1)|YEEEHAAAA !]]
